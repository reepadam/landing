...
- `invoices.json` — output across all 5 emailed invoice packages (26 invoice records; one multi-page H## P#### invoice merged into a single record)
...
5. **Line-item subtotal cross-validation + layered reconciliation.** After Claude returns a record, the script sums `quantity × unit_price` across line items and compares to `total_amount_due`. If divergence exceeds 1%, the script doesn't just flag and walk away — it tries to resolve the divergence through three layered escalations:
   ...
   a. **Vendor-specific parsing notes.** The vendor master allows each known vendor to carry a `parsing_notes` field that gets prepended as a re-extract instruction when that vendor's first pass fails. A#### P####### & R########### has a note explaining that their invoices place the line total in the "Unit Price" column on multi-quantity rows (so 500 × $45 Unit Price doesn't mean $22,500 — it means $45 total, $0.09 per unit). That note is the difference between flagging Angel's records and reconciling them. **All 5 A#### P####### records now reconcile within 1 cent. The parsing notes preserve 4-6 decimals on unit_price so the math holds exactly.**
   ...
   **Result across the full batch: all 26 invoice records pass cross-validation. The one record flagged for human review (R######### S#####) is not flagged for math reasons — it's flagged because the invoice was fetched via the email's link rather than received as an attached PDF, so we can't be 100% certain we captured every adjustment line on the source document. The script generates a vendor follow-up email draft explaining exactly what we need confirmed.** This is the architectural difference between "the script flags issues for humans" and "the script tries to solve them through layered controls, rejects fabrication, then flags only what it genuinely can't."
