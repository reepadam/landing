...
4. **Hybrid text + image input.** I caught a vendor-misidentification on the H## P########## Products invoices: text extraction put the bill-to ("HDS MARKETING/FACILIS GROUP") above the vendor letterhead, and the model named HDS as the vendor. Fix: every page is sent as **both** extracted text **and** rendered image, with the prompt instructing the model to defer to the image for layout-dependent fields. After the change all 7 H## P#### pages identify "H## P########## P#######, Inc." at high confidence.
5. **Line-item subtotal cross-validation + adjustment reconciliation.** After Claude returns a record, the script sums `quantity × unit_price` across line items and compares to `total_amount_due`. If divergence exceeds 1%, the script does NOT just flag and walk away — it runs a second Claude pass against the same document asking specifically: *"the line items I extracted sum to $X but the printed total is $Y; identify the adjustment lines (Freight, Tax, Shipping, Handling, Discount, Surcharge) that account for the $Z delta."* The reconciled adjustments are appended to the line items, and the subtotal check runs again. The R######### S##### invoice was the obvious case: line items summed to $437.76, total was $455.92, delta = $18.16. The reconciliation pass found Freight: $18.16 on the document and added it. Math now passes. **Result across the full batch: 22 of 27 records pass cross-validation cleanly; only 5 are flagged for human review — 4 from one vendor (A#### P#######) whose own invoice format puts the order total in the "Unit Price" column, and 1 with a missing line-item field. Real data quality issues, not script issues.** This is the architectural difference between "the script flags issues for humans" and "the script tries to solve the issues, then flags only what it genuinely can't."
